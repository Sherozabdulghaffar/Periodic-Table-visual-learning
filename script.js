...
let level1CertificateGiven = false;
...
async function loadSiteConfig() {
	try {
		...
		// Apply per-device overrides (from visible admin toggles)
...
function getDefaultTimerSeconds() {
	// Mobile users: 20 minutes, Desktop users: 10 minutes
	return isMobileLikeDevice() ? 20 * 60 : 10 * 60;
}
...
function checkLevelComplete() {
	...
	if (placedElements.length === elementsPerLevel.get(currentLevel)) {
		if (currentLevel < finalLevel) {
			// If the player just completed level 1, offer a certificate
			if (currentLevel === 1 && !level1CertificateGiven) {
				try {
					const name = prompt('Congratulations! You completed Level 1. Enter your name to receive a certificate:', 'Player');
					if (name !== null && name.trim() !== '') {
						generateCertificate(name.trim(), { autoDownload: true, showModal: true, level: 1 });
						level1CertificateGiven = true;
					}
				} catch (e) {
					console.warn('Level 1 certificate generation failed', e);
				}
			}
...
function handleStartReset() {
	...
	if (!isGameStarted) {
		...
		// Ensure grid exists and initialize first level (creates elements and starts timer)
		createDropZones();
...
function generateCertificate(nameParam, options = {}) {
	const { autoDownload = false, showModal = true, level = null, code = null, admin = false } = options || {}; // code: explicit certificate code (admin); admin: flag indicating admin-issued cert


	let name = nameParam;
	if (!name) {
		name = prompt('Enter your name for the certificate:', 'Player');
	...
	// Draw QR code (use Google Charts quick endpoint)
	const qrImg = new Image();
	qrImg.crossOrigin = 'Anonymous';
	qrImg.src = 'https://chart.googleapis.com/chart?cht=qr&chs=160x160&chl=' + encodeURIComponent(verificationURL) + '&chld=L|1';
	qrImg.onload = () => {
	...
	qrImg.onerror = () => {
		...
		// still save and download the certificate
		try {
			const storeKey = 'periodicCertificates';
			const raw = localStorage.getItem(storeKey);
			const map = raw ? JSON.parse(raw) : {};
			...
			localStorage.setItem(storeKey, JSON.stringify(map));
		} catch (err) { console.warn('Could not save certificate to localStorage', err); }
		const dataUrl = canvas.toDataURL('image/png');
		if (autoDownload) {
			const a = document.createElement('a');
			a.href = dataUrl;
